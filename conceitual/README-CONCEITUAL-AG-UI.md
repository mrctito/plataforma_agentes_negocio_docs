...
5. Como permitir que o agente desenhe visualizacoes (cards, graficos, tabelas) sob pedido, sem abrir a porta para HTML, JavaScript ou SQL livre gerado pela IA.
...
1. Existe um hub AG-UI de varejo com tres telas dedicadas: cockpit de vendas, radar de checkout e central de catalogo (um quarto card, de dashboard dinamico, existiu e foi removido junto com a materializacao visual do capability pack — secao 10.5).
...
Uma capability governada e uma intencao de negocio autorizada pelo backend. A tela diz "quero resumo de vendas" ou "quero funil de checkout". Ela nao manda SQL livre, HTML livre nem uma instrucao arbitraria para o browser executar.
...
6. Visualizacao generativa segura (A2UI) no chat embutivel, sob catalogo fechado de componentes.
...
E a camada hoje mais concreta em termos de experiencia visual pronta. Ela converte capabilities em consultas aprovadas — o caminho de materializacao de dashboard dinamico que existiu aqui foi removido do codigo.

### 10.6. [Runtime compartilhado do frontend](README-CONCEITUAL-AG-UI-RUNTIME-COMPARTILHADO-DO-FRONTEND.md)

E a camada que permite reuso entre paginas. Ela concentra cliente SSE, store, sidecar, renderer de tool timeline e o renderer A2UI (visualizacao generativa do chat embutivel).
...
4. O mesmo protocolo suporta consulta governada, visualizacao generativa (A2UI) e HIL.
...
O pacote interno @prometeu/ag-ui-runtime agora deve ser lido como wrapper fino de plataforma, nao como protocolo paralelo. Ele reexporta o cliente oficial da Plataforma de Agentes de IA sobre @ag-ui/client, tipos oficiais de @ag-ui/core, store, sidecar, renderers, timeline, Component Catalog seguro e helpers de integracao para auth, agent id, tenant, replay, diagnostico, catalogo e frontend tools. Nao ha mais validador de dashboard nesse pacote — o mecanismo que ele validava foi removido do codigo; a visualizacao generativa vigente (A2UI) tem seu proprio renderer fail-closed. Ele nao e publicado como pacote externo publico, mas ja funciona como fachada interna de reuso.
...
No caminho feliz, a pagina envia o run, o backend valida o contrato, o sidecar mostra status e correlation_id, a tool governada executa (ou o supervisor desenha A2UI quando o usuario pede visualizacao), o estado e reconstruido no frontend e o run termina com success. O usuario percebe uma interface viva, com feedback de processo, e nao um spinner opaco esperando resposta final.
...
5. A visualizacao generativa A2UI e flexivel dentro de um catalogo fechado de componentes, mas nao aceita HTML, script, SQL livre, segredo ou correlation_id no payload — o mecanismo antigo de dashboard dinamico do capability pack foi removido do codigo.
...
- Entendi que a visualizacao generativa A2UI usa uma especificacao segura (catalogo fechado de componentes), nao HTML livre.
...
- src/api/services/ag_ui_retail_demo_adapter.py
  ...
  - Comportamento confirmado: capabilities PDV fechadas (sales_summary, checkout_funnel, catalog_opportunities, customer_segments), bloqueio de SQL livre; zero ocorrencia de `dashboard`/`DashboardSpec` no arquivo. `src/api/services/ag_ui_dashboard_materialization.py` (o `DashboardMaterializationService` que existia neste fluxo) **nao existe mais no codigo** (confirmado por busca no repositorio).
