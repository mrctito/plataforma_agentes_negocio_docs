...
- Acoplamento forte com dominio?: Nao. Infraestrutura transversal de observabilidade.
- Uso atual observado: Sim. Consumido por [src/api/services/logs_admin_service.py](../src/api/services/logs_admin_service.py) e [src/api/routers/logs_router.py](../src/api/routers/logs_router.py) (`/logs/v1/analyze`). (O antigo consumidor `qa_system_server` não existe mais — removido com o eixo de hospedagem MCP.)
...
- Linguagem: Python
- Responsabilidade principal: expor `collect_declared_mcp_tool_ids` — centralizar a leitura/deduplicação dos ids MCP declarados no YAML, para o resolver de configuração (`mcp_config_resolver`) e o `workflow_semantic_validator`. Os fabricadores `build_declared_mcp_tool_entry`/`append_declared_mcp_tool_entries` foram REMOVIDOS.
